...
| **Coach del chat** | [backend/coach.py](backend/coach.py) | Chat de producto: Gemini REST con function calling y 5 herramientas sobre la BD. Es lo que responde en `/api/chat/coach`. |
| **Cliente Gemini** | [backend/gemini_client.py](backend/gemini_client.py) | Descubre el modelo disponible en la cuenta, cachea el elegido y guarda el último error para `/api/ai/diagnostics`. |
...
| **2026-09-11 22:40** | Claude (Opus 5) | `main` | **Peso unificado y cuota de Gemini identificada.** La app guardaba el peso en dos lugares (perfil y registro diario) y la tarjeta "Peso corporal" leía sólo del registro: por eso al decirle el peso al coach cambiaba "Datos de origen" pero no la tarjeta. Ahora el perfil es la fuente de verdad, el chat deja el peso también en el registro de hoy (aparece en el gráfico) y el formulario del dashboard actualiza el perfil con IMC y TDEE. Verificado en producción: "mi peso es 90" deja la tarjeta y el gráfico en 90. Además se detectó que la `GEMINI_API_KEY` del proyecto está en plan gratuito y su cuota diaria se agota: el chat ya no reintenta ante 429 (respondía a los 66 s), responde en menos de 1 s y avisa que contesta sin IA. Para la presentación conviene habilitar facturación en Google AI Studio o usar una clave con cuota. |
| **2026-09-11 21:55** | Claude (Opus 5) | `main` | **Chat verificado en producción.** Medición real con la API key del proyecto: `gemini-2.5-flash` y `gemini-2.0-flash` ya responden 404 para esta cuenta, y `gemini-3.6-flash` (el que estaba fijado en `GEMINI_MODEL` de Render) tarda **64 s** en contestar un saludo, por lo que toda petición caía al respaldo de reglas. Se cambió la variable a `gemini-flash-latest` (**1,3 s**), alias que Google mantiene apuntando al flash vigente. Prueba en vivo: "¿cuánto peso y cuál es mi meta?" responde con datos reales en 4,6 s usando `get_user_biometrics`; "ahora peso 90, guárdalo" persiste el peso y recalcula IMC 28.4 y TDEE 3238 en 6,3 s usando `update_user_profile`. También se corrigieron las métricas inventadas del panel admin (la tasa de actividad ahora se calcula con registros reales de los últimos 7 días). |
| **2026-09-11 21:30** | Claude (Opus 5) | `main` | **Chat arreglado de raíz.** Diagnóstico en producción: `/api/chat/coach` respondía siempre con reglas (`source: fallback`) porque el modelo estaba escrito a mano como `gemini-1.5-flash`, retirado por Google → 404 descartado en silencio; y `/api/ai/agent-chat` devolvía 503 porque importar LangChain agota la memoria del plan free de Render y reiniciaba el servicio. Se agregó `gemini_client.py` (descubre el modelo vía ListModels y cachea) y `coach.py` (function calling nativo por REST con las 5 herramientas reales sobre la BD). El chat de la web ahora lee y escribe perfil, registros y planes. Nuevos endpoints `/api/ai/diagnostics` y `/api/ai/diagnostics/probe`. `ENABLE_LANGCHAIN_AGENT` deja el agente SPEC-02 disponible sin tumbar el servicio. `DATABASE_URL` habilita PostgreSQL. Frontend: chat rediseñado (móvil a pantalla completa, chips de herramientas, reintento, historial), dashboard sin cifras inventadas y con gráfico real de peso, navegación móvil en el Navbar. 17/17 tests, lint, typecheck y build en verde. |
| **2026-09-11 04:30** | Claude (Sonnet 5) | `main` | SSO de Vercel desactivado por Sebastián, frontend ya público. Detectado y corregido: el agente del chat (`agent.py`/`mcp_server.py`) no tenía herramienta para modificar el perfil del usuario (peso, meta, altura, nivel de actividad) — solo podía leer datos y registrar calorías, por eso ignoraba pedidos como "guarda mi peso". Se agregó `update_user_profile` (con recálculo automático de IMC/TDEE y validación de rango fisiológico 20-400 kg / 100-250 cm) y se extendió `record_daily_log` para aceptar `weight_kg`. Se reforzó el system prompt para que el agente llame herramientas de forma obligatoria ante cualquier dato nuevo del usuario, en vez de solo dar consejo genérico. 17/17 tests pasando localmente. Commit `f1d8d1a`. |
...
4. [ ] **Persistencia real:** el backend en Render usa SQLite sobre disco efímero; lo que el usuario guarda (peso, registros, planes) se pierde en cada reinicio o despliegue. Crear un PostgreSQL administrado y definir `DATABASE_URL` en el servicio.
5. [ ] **Cuota de Gemini:** la clave está en plan gratuito y la cuota diaria se agota con pruebas. Habilitar facturación o rotar la clave antes de la presentación; `GET /api/ai/diagnostics` muestra `last_error_kind: quota` cuando ocurre.
6. [ ] **Verificar IA en producción:** `GET https://vitalcore-api.onrender.com/api/ai/diagnostics` debe mostrar `model_in_use` y `last_error: null`. Si no, revisar `GEMINI_API_KEY`.
7. [ ] **Salida a producción:** Completar los P0 de `AUDIT.md` (OAuth/OIDC, autorización por recurso y gobierno de datos de salud). Mantener `DEMO_MODE=true` hasta entonces.
